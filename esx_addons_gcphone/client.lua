...
RegisterNUICallback('send', function(data)
    ...
    TriggerEvent("pNotify:SendNotification", {text = "Mensagem enviada", type = "success", timeout = 1400, layout = "centerLeft"})
end)
...
Citizen.CreateThread(function()
    ...
    if #alerts ~= 0 then
      ...
      if alert.isAccept == 0 then
        AddTextComponentString('~o~Chamadas em progresso')
      else
        AddTextComponentString('~g~Chamadas em progresso')
      end
      DrawText(0.12, 0.02)
      ...
      SetTextColour(255, 255, 255, 255)
      SetTextEntry("STRING")
      AddTextComponentString(alert.isAccept .. ' Unidades')
      DrawText(0.195, 0.02)
      ...
      SetTextColour(255, 255, 255, 255)
      SetTextEntry("STRING")
      AddTextComponentString('Pressiona ~g~Y~s~ para atender a chamada ou ~r~N~s~ para rejeitar.')
      DrawText(0.025, 0.16)
      ...
      if alert.message ~= nil then
        ...
        local sh = 0.045
        while #message ~= 0 do
          message2 = string.sub(message, 0, 40)
          message2 = message2:gsub("^%s*(.-)%s*$", "%1")
          message = string.sub(message, 41)
          SetTextFont(0)
        ...
        SetTextColour(255, 255, 255, 255)
        SetTextEntry("STRING")
        AddTextComponentString("~o~Sem mensagem")
        DrawText(0.12, 0.08)
      end
      ...
      if alert.coords ~= nil then
        ...
        SetTextColour(255, 255, 255, 255)
        SetTextEntry("STRING")
        AddTextComponentString('~g~Localização: ~b~' .. math.floor(alert.coords.x) .. ' ' .. math.floor(alert.coords.y))
        DrawText(0.07, 0.14)
      else
        SetTextFont(0)
        SetTextScale(0.0, 0.24)
        ...
        SetTextColour(255, 255, 255, 255)
        SetTextEntry("STRING")
        AddTextComponentString('~o~Localização desconhecida')
        DrawText(0.12, 0.14)
      end
      ...
      if alert.numero ~= nil then
        ...
        SetTextColour(255, 255, 255, 255)
        SetTextEntry("STRING")
        AddTextComponentString('~o~Numero desconhecido')
        DrawText(0.17, 0.14)
      end
