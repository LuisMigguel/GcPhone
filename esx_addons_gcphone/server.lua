  ...
  if PhoneNumbers[number] ~= nil then
    for k,v in pairs(PhoneNumbers[number].sources) do
      getPhoneNumber(tonumber(k), function (n)
        TriggerEvent('gcPhone:_internalAddMessage', number, n, 'Alerta (' .. alert.numero  .. '): ' .. alert.message, 0, function (smsMess)
  ...
  if PhoneNumbers[type] ~= nil then
    ...
    for key, alert in pairs(PhoneNumbers[type].alerts) do
      ...
      if alert.id == alertId then
        ...
        TriggerClientEvent('esx_addons_gcphone:showMessage', tonumber(key), 'Unidades de ' .. PhoneNumbers[type].type .. ' a caminho.')
      ...
      if alert.id == alertId then
        ...
        if allRefuse == true then
          if PhoneNumbers[type].alerts[key].isAccept == 0 then
            TriggerClientEvent('esx_addons_gcphone:showMessage', tonumber(key), 'Todas as unidades estao ocupadas')
            for k, src in pairs(PhoneNumbers[type].alerts[key].sources) do 
              TriggerClientEvent('esx_addons_gcphone:showMessage', tonumber(k), 'Todas as unidades estao ocupadas')
            end
          end
