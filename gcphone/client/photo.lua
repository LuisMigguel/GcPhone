...
Citizen.CreateThread(function()
	...
	while true do
		...
		if phone == true then
			ESX.ShowNotification('Carrega na ~b~ "Seta para cima" ~s~ para trocares para o modo ~y~ selfie ~s~ Nova mensagem de.')
			ESX.ShowNotification('Carrega ~b~"ESC" ~s~ou ~b~"ESPACO" ~s~para ~r~sair ~s~ do modo de ~y~selfie~s~.')
			ESX.ShowNotification('Carrega ~b~"Botao esquerdo do rato" ~s~para ~g~tirar ~s~a ~y~foto~s~.')
